...
public class TestNGDataProviderwithExcel {
	public class ReadExcelDataProvider {
		 ...
		 * @throws Throwable 
		 */
		public String[][] getExcelData(String fileName, String sheetName) throws Throwable {
			...
			try {
				...
				int totalNoOfCols = sh.getRow(totalNoOfRows).getLastCellNum();
				arrayExcelData = new String[totalNoOfRows-1][totalNoOfCols];
				for (int i= 1 ; i < totalNoOfRows; i++) {

					for (int j=0; j < totalNoOfCols; j++) {
						// arrayExcelData[i-1][j] = sh.getCellComment(j, i).getContents();
					}
